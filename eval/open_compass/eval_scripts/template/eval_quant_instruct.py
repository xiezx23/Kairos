...
#       其中的eval.open_compass.models.quant_hf_model是QuantHuggingFacewithChatTemplate类的所在路径，需要根据实际情况配置
custom_imports = dict(imports=['eval.open_compass.models.quant_hf_model'], allow_failed_imports=False)
from eval.open_compass.models.quant_hf_model import QuantHuggingFacewithChatTemplate
########################
# 主要修改部分2
QUANT_LLM = True
...
QUANT_WEIGHT_PATH = model_root_path+"/quant_model/edq_model_Qwen2.5-7B-Instruct"
ABBR = "Qwen2.5-7B-Instruct_quant"  # 模型简称，用于结果展示
...
MAX_OUT_LEN = 4096
BATCH_SIZE = 8  # 根据显存调整, 量化模型的显存占用较低，可以适当调大batch size，如16
...
models = [
    ...
    dict(
        type=QuantHuggingFacewithChatTemplate,
