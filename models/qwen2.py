...
class Qwen2SdpaAttention(nn.Module):
    ...
    def forward(
        ...
        # FIXME: 临时检查, 原因参见源文件(transformers.models.qwen2.modeling_qwen2.Qwen2SdpaAttention, Line 511
...
class Qwen2Model(nn.Module):
    ...
    def forward(
        ...
        # FIXME: 验证past_key_values必定为Cache模式
        # assert past_key_values is None or isinstance(past_key_values, Cache), "past_key_values must be of type Cache"
        # if use_cache:
...
class Qwen2ForCausalLM(Qwen2ForCausalLM):
    ...
    def generate(
        ...
        # TODO: 暂定直接沿用原有的generate, 后续根据需求判断是否需要覆盖
