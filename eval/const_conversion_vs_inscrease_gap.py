...
input_len = [i for i in range(1, 1024, 128//2)]
...
if model_type == 'qwen':
    ...
    preheat_time = 300; n = 1000
    save_path = 'vit_single_const_conversion.pt'
    prof_path = './prof_vit/'
    base_m = 197
    shape_list = [(768, 768), (2304, 768), (3072, 768), (768, 3072)] 
elif model_type == 'bert':
    preheat_time = 100; n = 300
    save_path = 'bert_single_const_conversion.pt'
    prof_path = './prof_bert/'
    base_m = 197
    shape_list = [(768, 768), (2304, 768), (3072, 768), (768, 3072)] 
...
if __name__ == '__main__':
    ...
    for k in range(len(w_shape)):
        ax = plt.subplot(1, len(w_shape), k+1)
        def sub_list(a, b):
            c = []
        ...
        sub1 = sub_list(record[3][k], record[0][k]), # W4A16-FP16
        sub2 = sub_list(record[3][k], record[5][k]), # W8A8-W4A16
        ...
        # plt.plot(x, [1 for _ in range(len(record[1][k]d))], '--r' , linewidth=2, label = 'FP16    GEMM from CuBLAS')
        plt.plot(x, sub1, '-ok', linewidth=2, label = 'T(W4A16)-T(FP16)')
        # plt.plot(x, record[3][k], '-o', color='#2E75B6', linewidth=2, label = 'AWQ W4A16')
        # plt.plot(x, record[4][k], 's--',  color='#2E75B6', linewidth=2, label =  'MARLIN W4A16')
        # plt.plot(x, record[1][k], '-og', linewidth=2, label = 'Qserve W4A8')
    ...
    # plt.ylim(0.6, 2.6)
    # plt.grid(True)
    # plt.legend(ncol=1, loc='center', fontsize=14, handletextpad=0.3, columnspacing=0.7, handlelength=1.2, bbox_to_anchor=(-0.15, 1.1), frameon=False)
    ...
    # plt.savefig('result/single_qlinear_'+model_type+'.svg', dpi=150)
    # plt.show()
