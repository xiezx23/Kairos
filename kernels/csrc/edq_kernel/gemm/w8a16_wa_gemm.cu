...
struct KernelTraits {
    ...
    static constexpr int kSmemSizeS = size(SmemLayoutS{}) * sizeof(ScaleType) * 2;
...
__global__ void w8a16_wa_gemm(
    ...
    cutlass::half_t* __restrict__ B_zp,
    const int M,
    const int N,
    const int K
) {
    ...
    Tensor mZ = make_tensor(
        make_gmem_ptr(reinterpret_cast<ScaleType*>(B_zp)),
        make_shape(N),
        make_stride(Int<1>{})
    );
    ...
    Tensor tZgZ = thr_copy_S_g2s.partition_S(gZ);
    Tensor tZsZ = thr_copy_S_g2s.partition_D(sZ);
...
// if (tidx == 9 && n_block_idx ==0 && m_block_idx == 0) {
    ...
    dequantize_asym<KernelTraits::NUM_WARP_M, KernelTraits::NUM_WARP_N>(tCrB(_, _, mma_k_i), sS, sZ, lane_idx, warp_idx);
...
// if (tidx == 9 && n_block_idx ==0 && m_block_idx == 0) {
    ...
    for (int main_loop_i = 0; main_loop_i < main_loop_k; main_loop_i++) {
        ...
        for (int inner_loop_i = 0; inner_loop_i < inner_loop_k; inner_loop_i++) {
            ...
            dequantize_asym<KernelTraits::NUM_WARP_M, KernelTraits::NUM_WARP_N>(tCrB(_, _, inner_loop_next), sS, sZ, lane_idx, warp_idx);
...
#define LAUNCH_KERNEL                                                           \
    ...
    BOOL_SWITCH(is_even_m, IS_EVEN_M, [&] {                                    \
        ...
        dim3 block_dim(KTraits::WARP_SIZE, NUM_WARPS);                         \
                                                                               \
        auto kernel = w8a16_wa_gemm<KTraits>;                                  \
...
void w8a16_wa_gemm_dispatch(
    ...
    cutlass::half_t* __restrict__ w_zp,
    const int token,
...
at::Tensor w8a16_wa_gemm_cuda(
    ...
    TORCH_CHECK(qweight.size(-2) == w_zp.size(-1));
    // printf("w8a16_wa_gemm_cuda\n");
    ...
    auto w_zp_ptr = reinterpret_cast<cutlass::half_t*>(w_zp.data_ptr());
    ...
    DISPATCH_WEIGHT_DTYPE(qweight.scalar_type(), weight_type, {
        ...
        w8a16_wa_gemm_dispatch(
            qact_ptr, qweight_ptr, out_feat_ptr, 
            w_scale_ptr, w_zp_ptr,
            token, out_channel, in_channel
        );
    });
