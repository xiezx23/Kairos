...
)
{
    ...
    int parallel = 1;   //? parallel in m_tiles
    ...
    //! Marlin 采用 stream-K 来划分每个 SM 的工作量，每个SM计算 iters 个 [k,n] tile，可能会跨 slice_col
    int iters = ceildiv(k_tiles * n_tiles * parallel, gridDim.x);   //? gridDim.x = Num(SM)
    ...
    //! slice 是在 B 矩阵上进行的划分 [k_tiles, n_tiles]，在 K 上连续，不同的 slice_col 代表了不同的slice
    //!   0 1 2         例如对于左边的一个 B 分块 (4,3)，thread_k_blocks = 4， thread_n_block = 3
    //!   0 1 3         此时 Num(SM) = 4，因此均分下来每个 SM 计算量 12 / 4 = 3
    //!   0 2 3         每一列视为一个 slice，共有3个slice，每个slice计算得到对应 C 分块的结果
    //!   1 2 3         可以发现每个SM可能会跨slice， 而每个slice也可能由不同的SM处理，涉及额外的地址转换和同步开销
    ...
    {
        A += (slice_col_par / n_tiles) * 16 * thread_m_blocks * prob_k / 8; //? "div 8" is because ptr is int32*4 (containing 8 fp16)
    ...
    //! 计算完每个分块后，更新地址索引和同步计数
    //!     slice_iters : 当前 CTA 在当前 slice 中要处理的 tile 数量
    //!     slice_count : 当前 slice 被多少活跃线程块协同处理
    //!     slice_idx   : 当前线程块在 slice 中的索引
    auto init_slice = [&]()
    {   //! 计算在当前 slice 中剩余的工作量
        slice_iters = iters * (blockIdx.x + 1) - (k_tiles * slice_col_par + slice_row);
        if (slice_iters < 0 || slice_col_par >= n_tiles * parallel)
            slice_iters = 0;    //? 对于最后一个CTA，排除无效 tile
        if (slice_iters == 0)
            return;
        if (slice_row + slice_iters > k_tiles)  //! 说明剩余的 tile 跨 slice
            slice_iters = k_tiles - slice_row;
        slice_count = 1;
        slice_idx = 0;
        //! col_first 代表第一个处理该 slice 的CTA处理结束的 tile 全局索引
        int col_first = iters * ceildiv(k_tiles * slice_col_par, iters);
        if (col_first <= k_tiles * (slice_col_par + 1)) //! else 说明该 slice 被一个CTA全包，不需要同步
        {   //! col_off : 第一个CTA在该slice上处理的tile数
...
// We have the m dimension as the inner loop in order to encourage overlapping dequantization and matmul operations.
    ...
    #pragma unroll
            for (int i = 0; i < thread_m_blocks; i++)
            {   //* sub tile: m16n16k16
                mma(frag_a[k % 2][i], frag_b0, frag_c[i][j][0]);    //* m16n8k16 * 2 in n-dimension to use the same fragA
...
#define CALL_IF(THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS, GROUP_BLOCKS)                                                        \
    ...
    {                                                                                                                                   \
        cudaFuncSetAttribute(                                                                                                           \
            ...
            SHARED_MEM);                                                                                                                \
        Marlin<                                                                                                                         \
            THREADS, THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS, STAGES, GROUP_BLOCKS><<<blocks, THREADS, SHARED_MEM, stream>>>( \
...
int marlin_cuda(
    ...
    int max_par = 16)
{   /*
    *Note: The shape of mma instruction: m16n8k16, and Marlin use a sub tile: m16n16k16
    */
    int tot_m = prob_m;
    ...
    int blocks = sms;   //* Marlin 采用 stream-k，线程块数 = num(SM)
    ...
    {   //*Note: thread_m_blocks表示一个 tile 在 m 维度上需要的 sub_tile 个数，最大为4，故 max_tile_m = 64
        //*      par: parallel factor，最大为16
        int thread_m_blocks = tot_m_blocks - i;
        prob_m = tot_m - 16 * i;
        int par = 1;    //? par \in [1, 16]
        if (thread_m_blocks > 4)
        {
            ...
            if (par > max_par)
                par = max_par;
            prob_m = 64 * par;  //* 一个kernel最大处理的 m = max_tile_m * max_par = 1024
            i += 4 * (par - 1);
            thread_m_blocks = 4;
        }
