# RUN: python -m test.compare_kernel_KN
...
def test(K, N):
    ...
    for m in input_len:
        ...
        # for _ in range(5):
        #     output4 = torch.empty(input.shape[0], weight.shape[0], device='cuda', dtype=torch.float16)
        #     edq_cuda_accel.w8a16_ws_gemm_cuda(input, weight_int8, scale_w, output4)
        ...
        print('loss1:', loss.item()/aver.item())
        loss = (refout-output2).abs().mean()
        print('loss2:', loss.item()/aver.item())
    ...
    plt.figure()
    # w8a8_speed = min(recordList[0], recordList[2])
    # w8a16_speed = min(recordList[1], recordList[3])
    # plt.plot(w8a8_speed, color='r')
    # plt.plot(w8a16_speed, color='b')
