...
class DynamicLinearManager():
    ...
    def append_linear(self, d_linear:"DynamicLinear"):
...
class DynamicLinearSyn(torch.nn.Module):
    """
    ...
    @classmethod
    def from_module(cls, linear, device, return_dtype=torch.float16, name = 'dynamic_linear',init_only=False):
        q_linear = cls(
        ...
        # q_linear.weight_i8 = dequantize_tensor_int4(
        #         weight_i4, q_linear.scale_2_i, 
    ...
    def _dynamic_trans_weight(self):
        ...
        #     self.weight = torch.empty(self.shape, dtype=torch.float16, device='cuda')
        # self.weight = torch.empty(self.shape, dtype=torch.float16, device='cuda')
        ...
        ###########################################################################
        # if self.weight is None:
        #     self.weight = torch.empty(self.shape, dtype=torch.int8, device='cuda')
    ...
    @torch.no_grad
    def _evict_previous_linear(self):
        pre_dl = global_DLM.get_linear((self.linear_idx - 1 + global_DLM.size) % global_DLM.size)
        pre_dl._evict_weight()
    
    @torch.no_grad
    def _prepare_next_linear(self):
        # nvtx.range_push(f"DynamicLinear_{self.linear_idx}:_prepare_next_linears")
        next_dl = global_DLM.get_linear((self.linear_idx + 7) % global_DLM.size)
        next_dl._dynamic_trans_weight()
        # nvtx.range_pop()
    ...
    def forward(self, input:torch.Tensor) -> torch.Tensor:
        ...
        self._evict_previous_linear()
        out = self._forward(input)
        self._prepare_next_linear()
        # nvtx.range_pop()
        return out

    @torch.no_grad
    def _forward(self, input:torch.Tensor) -> torch.Tensor:
        # nvtx.range_push(f"DynamicLinear_{self.linear_idx}:_forward")
        ...
        # out = input @ self.ori_weight

        if self.bias is not None:
            out += self.bias
        # out = out.view(intput_shape[0], intput_shape[1], -1)
        # nvtx.range_pop()
        ...
        #     print(input)
        #     print(self.weight)
        #     print(self.ori_weight)
        #     assert(0)
        # assert torch.isinf(out).sum() == 0, print(self.name)
        return out
