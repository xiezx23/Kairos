...
from thirdparty.AutoGPTQ.w4a16_linear import mul
...
class DynamicLinear(torch.nn.Module):
    ...
    def from_module(cls, linear, device, return_dtype=torch.float16, init_only=False, name = ''):
        ...
        """ Step0: Store weight for Marlin W4A16.   """
        from thirdparty.AutoGPTQ.w4a16_linear import W4A16Linear_Marlin
        q_linear.marlin = W4A16Linear_Marlin.from_module(linear, device, init_only)
        from thirdparty.Qserve.w4a8_linear import W4A8Linear
        q_linear.qserve = W4A8Linear.from_module(linear, device, init_only)
    ...
    def _forward(self, input:torch.Tensor, batch_token_len, comp_type) -> torch.Tensor:
        ...
        if comp_type == COMP_TYPE_NAME['w4a16_gemv']:
            ...
            out = awq_backend.gemm_forward_cuda_new(
                input, self.qweight, self.scales, self.scaled_zeros)
        elif comp_type == COMP_TYPE_NAME['w4a16_marlin']:
            out = torch.empty(input.shape[:-1] + (self.marlin.s.shape[1],), dtype=input.dtype, device=input.device)
            mul(
                input.view((-1, input.shape[-1])),
                self.marlin.B,
                out.view((-1, out.shape[-1])),
                self.marlin.s,
                self.marlin.workspace,
            )
        elif comp_type == COMP_TYPE_NAME['w4a8_qserve']:
            ...
            tmp_weight = kairos_cuda_accel.trans_layout_c16_to_c8_awq(self.qweight, self.out_features, self.in_features)
            ...
            omniserve_backend.qgemm_w4a8_per_group.gemm_forward_cuda(
                        int_x,
                        tmp_weight,
                        self.qserve.s2_zeros,
                        self.qserve.s2_scales,
                        self.qserve.s1_scales,
                        scale_x,
                        out,
                    )
        # w8a8 kernel
