    ...
    DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(data_type, ctype, {
      ...
      if (group_size == 128)
      {
        ...
        case 7:
          ...
          break;
        case 8:
          gemv_kernel<N_PER_BLOCK, 8, BLOCK_SIZE, 128><<<num_blocks, num_threads>>>(
            in_feats, kernel, scaling_factors, zeros, out_feats, k, n
          );
          break;
        case 9:
          gemv_kernel<N_PER_BLOCK, 9, BLOCK_SIZE, 128><<<num_blocks, num_threads>>>(
            in_feats, kernel, scaling_factors, zeros, out_feats, k, n
          );
          break;
        case 10:
          gemv_kernel<N_PER_BLOCK, 10, BLOCK_SIZE, 128><<<num_blocks, num_threads>>>(
            in_feats, kernel, scaling_factors, zeros, out_feats, k, n
          );
          break;
        case 11:
          gemv_kernel<N_PER_BLOCK, 11, BLOCK_SIZE, 128><<<num_blocks, num_threads>>>(
            in_feats, kernel, scaling_factors, zeros, out_feats, k, n
          );
          break;
        case 12:
          gemv_kernel<N_PER_BLOCK, 12, BLOCK_SIZE, 128><<<num_blocks, num_threads>>>(
            in_feats, kernel, scaling_factors, zeros, out_feats, k, n
          );
          break;
        case 13:
          gemv_kernel<N_PER_BLOCK, 13, BLOCK_SIZE, 128><<<num_blocks, num_threads>>>(
            in_feats, kernel, scaling_factors, zeros, out_feats, k, n
          );
          break;
        case 14:
          gemv_kernel<N_PER_BLOCK, 14, BLOCK_SIZE, 128><<<num_blocks, num_threads>>>(
            in_feats, kernel, scaling_factors, zeros, out_feats, k, n
          );
          break;
        case 15:
          gemv_kernel<N_PER_BLOCK, 15, BLOCK_SIZE, 128><<<num_blocks, num_threads>>>(
            in_feats, kernel, scaling_factors, zeros, out_feats, k, n
          );
          break;
        case 16:
          gemv_kernel<N_PER_BLOCK, 16, BLOCK_SIZE, 128><<<num_blocks, num_threads>>>(
            in_feats, kernel, scaling_factors, zeros, out_feats, k, n
          );
          break;
        case 17:
          gemv_kernel<N_PER_BLOCK, 17, BLOCK_SIZE, 128><<<num_blocks, num_threads>>>(
            in_feats, kernel, scaling_factors, zeros, out_feats, k, n
          );
          break;
        case 18:
          gemv_kernel<N_PER_BLOCK, 18, BLOCK_SIZE, 128><<<num_blocks, num_threads>>>(
            in_feats, kernel, scaling_factors, zeros, out_feats, k, n
          );
          break;
        case 19:
          gemv_kernel<N_PER_BLOCK, 19, BLOCK_SIZE, 128><<<num_blocks, num_threads>>>(
            in_feats, kernel, scaling_factors, zeros, out_feats, k, n
          );
          break;
        case 20:
          gemv_kernel<N_PER_BLOCK, 20, BLOCK_SIZE, 128><<<num_blocks, num_threads>>>(
            in_feats, kernel, scaling_factors, zeros, out_feats, k, n
          );
          break;
        case 21:
          gemv_kernel<N_PER_BLOCK, 21, BLOCK_SIZE, 128><<<num_blocks, num_threads>>>(
            in_feats, kernel, scaling_factors, zeros, out_feats, k, n
          );
          break;
        case 22:
          gemv_kernel<N_PER_BLOCK, 22, BLOCK_SIZE, 128><<<num_blocks, num_threads>>>(
            in_feats, kernel, scaling_factors, zeros, out_feats, k, n
          );
          break;
        case 23:
          gemv_kernel<N_PER_BLOCK, 23, BLOCK_SIZE, 128><<<num_blocks, num_threads>>>(
            in_feats, kernel, scaling_factors, zeros, out_feats, k, n
          );
          break;
        case 24:
          gemv_kernel<N_PER_BLOCK, 24, BLOCK_SIZE, 128><<<num_blocks, num_threads>>>(
            in_feats, kernel, scaling_factors, zeros, out_feats, k, n
          );
          break;
        case 25:
          gemv_kernel<N_PER_BLOCK, 25, BLOCK_SIZE, 128><<<num_blocks, num_threads>>>(
            in_feats, kernel, scaling_factors, zeros, out_feats, k, n
          );
          break;
        case 26:
          gemv_kernel<N_PER_BLOCK, 26, BLOCK_SIZE, 128><<<num_blocks, num_threads>>>(
            in_feats, kernel, scaling_factors, zeros, out_feats, k, n
          );
          break;
        case 27:
          gemv_kernel<N_PER_BLOCK, 27, BLOCK_SIZE, 128><<<num_blocks, num_threads>>>(
            in_feats, kernel, scaling_factors, zeros, out_feats, k, n
          );
          break;
        case 28:
          gemv_kernel<N_PER_BLOCK, 28, BLOCK_SIZE, 128><<<num_blocks, num_threads>>>(
            in_feats, kernel, scaling_factors, zeros, out_feats, k, n
          );
          break;
        case 29:
          gemv_kernel<N_PER_BLOCK, 29, BLOCK_SIZE, 128><<<num_blocks, num_threads>>>(
            in_feats, kernel, scaling_factors, zeros, out_feats, k, n
          );
          break;
        case 30:
          gemv_kernel<N_PER_BLOCK, 30, BLOCK_SIZE, 128><<<num_blocks, num_threads>>>(
            in_feats, kernel, scaling_factors, zeros, out_feats, k, n
          );
          break;
        case 31:
