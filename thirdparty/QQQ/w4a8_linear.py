...
class W4A8Linear_QQQ(nn.Module):
    @torch.no_grad
    def __init__(
        self, group_size, infeatures, outfeatures, bias, **kwargs
    ):
        super().__init__()
        ...
        if group_size == -1:
            assert 0, print('Only support Per-group now.')
    ...
    def pack(self, linear, scales, s_extra=None):
        ...
        if self.group_size != self.infeatures:
            ...
            s_extra = s_extra.reshape((-1, len(_scale_perm_single)))[
                :, _scale_perm_single
            ]
            ...
            s = (
                (s / (2 ** (8 - self.bits)))
                .reshape((-1, len(_scale_perm_single)))[:, _scale_perm_single]
    ...
    @classmethod
    def from_module(cls, linear, device='cuda', return_dtype=torch.float16, init_only=False, name = ''):
        q_linear = cls(128, linear.in_features, linear.out_features, linear.bias is not None)
        q_linear.cuda()
        if init_only: 
            return q_linear
        # Symmetric 4bit Quantization
        qw4, qconfig4 = quantize_tensor_int4(linear.weight.data.clone(), 'S', 128)
        qw8, qconfig8 = quantize_tensor_int8(linear.weight.data.clone(), 'S', -1)
        q_linear.pack(linear, qconfig4['scale'].view(linear.out_features, -1), qconfig8['scale'].T.contiguous().to(float))
        return q_linear
