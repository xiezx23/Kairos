...
__global__ void dequant_interleaved_int4_to_fp16_kernel(
    ...
    for (int i = tid; i < num_groups * interleave; i += blockDim.x) {
        ...
        // const int bias = row_block * interleave;
        // if (bias + i%interleave < N) {
        //     shared_scale[i]       = scale[i + bias];
        //     shared_scaled_zero[i] = scaled_zero[i + bias];
        // }
        #endif
    }
    __syncthreads();
    ...
    for (int cur_tid = tid; cur_tid < solve_end; cur_tid += blockDim.x) {
        ...
        for (int row_in_block = 0; row_in_block < interleave; row_in_block++) {
            ...
            __half* output_ptr = output + act_row * K;
            ...
            output_ptr[_idx+ 0+elems_bidx] = val0; output_ptr[_idx+ 1+elems_bidx] = val1;
            output_ptr[_idx+ 8+elems_bidx] = val2; output_ptr[_idx+ 9+elems_bidx] = val3;
            output_ptr[_idx+16+elems_bidx] = val4; output_ptr[_idx+17+elems_bidx] = val5;
            output_ptr[_idx+24+elems_bidx] = val6; output_ptr[_idx+25+elems_bidx] = val7;
        }
    }
}
...
void dequant_interleaved_int4_to_fp16(
    ...
    dequant_interleaved_int4_to_fp16_kernel<<<grid, block, shared_mem_size, stream>>>(
