...
template <const int threads,         // number of threads in a threadblock
          ...
          const int stages, // number of stages for the async global->shared
                            // fetch pipeline
          const int group_blocks = 128 // number of consecutive 16x16 blocks with
                                      // a separate quantization scale
          >
__global__ void
w4a16_wa_gemm(const int4 *__restrict__ A, // fp16 input matrix of shape mxk
       const int4 *__restrict__ B, // 4bit quantized weight matrix of shape kxn
       int4 *__restrict__ C,       // fp16 output buffer of shape mxn
       const int4
           *__restrict__ s, // fp16 quantization scales of shape (k/groupsize)xn
       const int4 
           *__restrict__ z, // fp16 scale_zero_point of shape (k/groupsize)xn
  ...
  // par
  int parallel = 1;
  if (prob_m > 16 * thread_m_blocks) { // par > 1
...
#pragma unroll
  ...
  // TODO: 这里的warp-level reduce好像有随机性
  // 可以考虑把这个改为确定性的warp-level reduce
...
#pragma unroll
                for (int k = 0; k < 4; k++) // 随机性导致这里的计算不一样
...
#pragma unroll
  ...
  // TODO: 这里的global reduce可能也有随机性的问题
...
#pragma unroll
  ...
  auto write_result = [&]() {
    ...
    // per-channel在这里进行反量化 但非对称不能这么做 也应该是先反量化后计算
...
const int SHARED_MEM =
    163 * 1024; // max shared memory on compute capability 8.0
...
#define CALL_IF(THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS,             \
    ...
    cudaFuncSetAttribute(w4a16_wa_gemm<THREADS, THREAD_M_BLOCKS, THREAD_N_BLOCKS,     \
                                THREAD_K_BLOCKS, STAGES, GROUP_BLOCKS>,        \
                         cudaFuncAttributeMaxDynamicSharedMemorySize,          \
                         SHARED_MEM);                                          \
    w4a16_wa_gemm<THREADS, THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS, STAGES, \
           GROUP_BLOCKS><<<blocks, THREADS, SHARED_MEM, stream>>>(             \
        A_ptr, B_ptr, C_ptr, s_ptr, z_ptr, prob_m, prob_n, prob_k, locks);            \
  }


int w4a16_wa_gemm_dispatch(const void *A, const void *B, void *C, void *s, void *z, int prob_m,
                int prob_n, int prob_k, void *workspace, int groupsize = 128,
...
void w4a16_wa_gemm_cuda(const torch::Tensor &A, const torch::Tensor &B, torch::Tensor &C,
         const torch::Tensor &s, const torch::Tensor &z, torch::Tensor &workspace, int thread_k,
  ...
  int err = w4a16_wa_gemm_dispatch(A.data_ptr(), B.data_ptr(), C.data_ptr(), s.data_ptr(), z.data_ptr(),
