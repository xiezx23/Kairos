...
comp_type_list = ['W4A16 GEMM', 'W4A16 GEMV', 'W8A8 GEMM', 'w8a16_gemm', 'w8a16_gemv', 'FP16 GEMM', 'W4A16 MARLIN']
...
input_len = [i for i in range(1, 8192, 512)]
...
def test(i, K, N):
    ...
    # for i in range(5):
    #     record[i] = [item for sublist in record[i] for item in sublist]

    def showStack():
        plt.plot(input_len, record[4], '-og', linewidth=2, label = 'W4A8  from Qserve')
        plt.plot(input_len, record[3], '-o', color='#2E75B6' ,linewidth=2, label = 'W4A16 from AWQ')
        ...
        plt.stackplot(input_len, record[0], record[1], record[2], Decision, 
                      labels=["mpGEMM in Kairos","Actvation Quantization","Weight Conversion", "Decision-making"], 
                      colors=['#abd9e9','#2c7bb6', '#d7191c', "#e6ab02"])
        plt.legend(ncol=2,handletextpad=0.3, columnspacing=0.7, handlelength=1,  frameon=True)
...
if __name__ == '__main__':
    ...
    plt.savefig(f'./breakdown.pdf', format='pdf', dpi=300, bbox_inches='tight')
    plt.savefig(f'./breakdown.png', format='png', dpi=300, bbox_inches='tight')
    plt.show()