...
class W4A16Linear_Marlin(nn.Module):
    @torch.no_grad
    def __init__(self, group_size, infeatures, outfeatures, bias, **kwargs):
        super().__init__()
    ...
    def pack(self, linear, scales):
        ...
        # return w.reshape(self.group_size, -1, self.outfeatures).permute(1,0,2).reshape(self.infeatures, -1)
        w += (maxq + 1) // 2
    ...
    def from_module(cls, linear, device = 'cuda', return_dtype=torch.float16, init_only=False, name = ''):
        ...
        qw, qconfig = quantize_tensor_int4(linear.weight.data.clone(), 'S', 128)
        q_linear.pack(linear, qconfig['scale'].view(linear.out_features, -1))
        # assert (qw-w.T).abs().sum() == 0, print((qw-w.T).abs().sum(), (qw-w.T).abs().max())
        return q_linear
