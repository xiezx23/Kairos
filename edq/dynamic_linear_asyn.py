...
class DynamicLinearManager():
    ...
    def get_stream(self, linear_name):
        if self.stream_pool.get(linear_name) is None:
            self.stream_pool[linear_name] = torch.cuda.Stream()
        return self.stream_pool[linear_name]
    ...
    def initial_prepare_weight(self):
        for idx in range(prepare_k):
            self.linear_pool[idx]._dynamic_trans_weight()
...
class DynamicLinear(torch.nn.Module):
    ...
    """
    @torch.no_grad
    def __init__(self, in_features, out_features, bias, device, group_size, name):
        super().__init__()
    ...
    def init(self):
        """ Initial work for quantized model. """
    ...
    def from_module(cls, linear, device, return_dtype=torch.float16, name = '',init_only=False):
        q_linear = cls(
            ...
            linear.bias is not None, device, group_size = 128, name=name)
    ...
    def _dynamic_trans_weight(self):
        ...
        ###########################################################################
        if self.weight is None:
            self.weight, self.widx = global_TP.get_tensor(self.shape, torch.int8)
        # with torch.cuda.stream(self.stream):
        ...
        event_list[self.linear_idx].record(self.stream)
    ...
    def _evict_weight(self):
        if self.weight is not None:
            # self.weight[:] = 0
            global_TP.free_tensor(self.shape, self.weight.dtype, self.widx)
            self.weight = None
    ...
    def forward(self, input:torch.Tensor) -> torch.Tensor:
        ...
        self._evict_weight()
        global_DLM.trans_weight((self.linear_idx + prepare_k) % global_DLM.size)
        # nvtx.range_pop()
        return out
    ...
    def _forward(self, input:torch.Tensor) -> torch.Tensor:
        ...
        event_list[self.linear_idx].synchronize()
